...
abstract class " . $this->getUnqualifiedClassName() . " extends " . $parentClass . "
{
    ...
    protected function addClassBody( &$script )
    {
        ...
        // namespaces
        $this->declareClasses(
                '\Propel\Runtime\Propel', '\Propel\Runtime\ActiveQuery\ModelCriteria', '\Propel\Runtime\ActiveQuery\Criteria', '\Propel\Runtime\ActiveQuery\ModelJoin', '\Exception', '\Propel\Runtime\Exception\PropelException'
        );
...
";
    ...
    protected function addFindPkSimple( &$script )
    {
        ...
        $query = sprintf(
                'SELECT %s FROM %s WHERE %s', implode( ', ', $selectColumns ), $this->quoteIdentifier( $table->getName() ), implode( ' AND ', $conditions )
        );
        $pks = array();
        if ( $table->hasCompositePrimaryKey() )
        {
...
";
    ...
    {
        $this->declareClasses(
                '\Propel\Runtime\Collection\ObjectCollection', '\Propel\Runtime\Connection\ConnectionInterface', '\Propel\Runtime\Propel'
        );
        $table = $this->getTable();
        $pks = $table->getPrimaryKey();
        $count = count( $pks );
...
";
    ...
    {
        $this->declareClasses(
                '\Propel\Runtime\Collection\ObjectCollection', '\Propel\Runtime\Exception\PropelException'
        );
        $table = $this->getTable();
        $queryClass = $this->getQueryClassName();
